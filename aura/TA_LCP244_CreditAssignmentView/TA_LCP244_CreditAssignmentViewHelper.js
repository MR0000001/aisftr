({
    doInit : function(component,event,helper) {
        ...
        console.log('general @@>> ' + JSON.stringify(component.get("v.general")));
        component.set('v.custom', JSON.parse(component.get('v.fieldSet')).custom);
        console.log('custom @@>> ' + JSON.stringify(component.get("v.custom")));
        component.set('v.customerCardBg', $A.get('$Resource.TA_Images') + '/imgs/' + component.get('v.general.bgImage'));
        console.log('customerCardBg @@>> ' + JSON.stringify(component.get("v.customerCardBg")));
        ...
        action.setCallback(this,function(response){
            if(response.getState()==="SUCCESS"){
                ...
                orderItems.forEach(function(orderitem) {
                    if(!orderitem.hasAdditionalFieldsFilled){
                        isValid = false;
                    }
                });

                this.fireSendInitStateEvt(component, true);            
                this.fireValidationEvent(component,isValid);
    ...
    fireSendInitStateEvt : function(component, isInitialized) {
        console.log('TA_LCP244_CreditAssignmentView >> Helper >> fireSendInitStateEvt >> Start');        
        let sendInitStateEvt = component.getEvent("sendInitStateEvt");
        sendInitStateEvt.setParams({
            "componentName" :  "TA_LCP244_CreditAssignmentView",
            "initState"   :  isInitialized
        });
        sendInitStateEvt.fire();
        console.log('TA_LCP244_CreditAssignmentView >> Helper >> fireSendInitStateEvt >> End');
    },

    fireValidationEvent : function(component, isValid) {
        console.log('TA_LCP244_CreditAssignmentView >> Helper >> fireEvts >> Start');    
        console.log('isValid: ' + isValid);    
        let validationEvt = $A.get("e.c:TA_LCE199_Validation");
        validationEvt.setParams({
            "cmpName" : 'TA_LCP244_CreditAssignmentView',
            "errors" : [$A.get("$Label.c.TA_CreditAssignmentFillFields")],
            "validate" : isValid
        });
        validationEvt.fire();
        console.log('TA_LCP244_CreditAssignmentView >> Helper >> fireEvts >> End');
    },
