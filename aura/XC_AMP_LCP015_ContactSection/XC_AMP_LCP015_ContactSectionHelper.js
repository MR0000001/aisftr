({
    ...
    loadPredefaultValues: function (component, event, helper) {
        ...
        let inputCmps = component.find('field');
        inputCmps.forEach((currentInput) => {
            if (currentInput.get("v.fieldName") === "XC_ContactDocumentCountry__c") {
                currentInput.set("v.value", component.get("v.extraFields.userCountry"));
            } else if (currentInput.get("v.fieldName") === "XC_Preferredlanguageofcontact__c") {
                currentInput.set("v.value", component.get("v.extraFields.preferredLanguage"));
            }
        });
