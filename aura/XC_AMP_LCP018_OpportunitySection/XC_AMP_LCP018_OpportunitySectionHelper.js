({
	doInit : function(component, event, helper) {
        ...
        let defaultCategory = component.get("v.categoryOfInterest");
        fieldsList.forEach((currentInput)=>{
            if(currentInput.fieldName==="XC_ProductCategoryOfInterest__c" && !$A.util.isEmpty(defaultCategory)){
            	currentInput.isReadonly = true;
            } else if(currentInput.fieldName==="XC_ProductCategoryOfInterest__c" && ! !$A.util.isEmpty(defaultCategory)) {
                currentInput.isReadonly = false;
            }
        });
    ...
    loadPredefaultValues : function(component,event,helper){
    	...
    	if(Array.isArray(inputCmps)){
            inputCmps.forEach((currentInput)=>{
                if(currentInput.get("v.fieldName")==="XC_ProductCategoryOfInterest__c" && !$A.util.isEmpty(defaultCategory)){
                    currentInput.set("v.value", defaultCategory);
                }
            });
        } else {
            if(inputCmps.get("v.fieldName")==="XC_ProductCategoryOfInterest__c" && !$A.util.isEmpty(defaultCategory)){
               inputCmps.set("v.value", defaultCategory);
            }
        }
 	...
 	validateData : function(component,event,helper){
        ...
        console.log('Opportunity information validity ' + isAllValid);
