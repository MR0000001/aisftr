({
    ...
    init: function (component, event, helper) {
        ...
        var action = component.get("c.initPaymentPageForOrder");

        action.setParams({
            "accountId": component.get("v.firstRecordId"),
            "objectName": component.get("v.sobjecttype"),
            "orderId": component.get("v.configurationId"),
            ...
            if (state === "SUCCESS" && res) {
                ...
                if (valueMap.billingContacts) {
                    (valueMap.billingContacts).forEach(function (entry) {
                        ...
                        if(entry.hasOwnProperty('email') && entry['email']!=null && entry['email']!='' && entry['email']!=undefined && entry.hasOwnProperty('primary') && entry['primary'] == 'true') {
                            component.set('v.notExistsEmail', false);
                            component.set('v.contactHasEmail', true);
                            console.log('@@@ Primary contact has email: ' + entry['email']);
                        }
                    })
                ...
                if (valueMap.contracts) {
                    component.set("v.mapContractIdContractType", valueMap.contracts);
                    let contractOpt = [];

                    (valueMap.contracts).forEach(function (entry) {
                        contractOpt.push({
                            value: entry['key'],
                            label: entry['value']
                        });
                    })
                    component.set("v.contractOptions", contractOpt);
                    console.log('setto valueMap.gasCUP = ' + valueMap.gasCUP);
                    component.set("v.gasCUP", valueMap.gasCUP);
                    component.set("v.electricCUP", valueMap.electricCUP);
                    console.log('setto valueMap.electricCUP = ' + valueMap.electricCUP);
                ...
                } else if (component.get("v.recordTypeName") == "XC_GLO_Account_Condominium" ||component.get("v.recordTypeName") == 'XC_GLO_Account_Soho'|| component.get("v.recordTypeName") == 'XC_GLO_B2B' || component.get("v.recordTypeName") == 'XC_GLO_B2G' ) {
                ...
                if (component.get("v.closeStrikeModal")) {
                    ...
                    //check on contacts
                    let soldContactsIds = valueMap.soldContacts.map(x => {return x.key});
                    if(soldContactsIds.includes(component.get("v.contactId"))){
                        component.set("v.valueMap.soldContact", component.get("v.contactId"));
                    }else{
                        component.set("v.valueMap.soldContact", soldContactsIds[0]);
                    }

                    component.set("v.valueMap.billingAddress", component.get("v.addressId"));
                    component.set("v.valueMap.soldAddress", addressOpts2[0].value);
                    component.set("v.valueMap.legal", component.get("v.legalEnt"));
                    component.set("v.showContact", true);

                    //check on shipping address
                    if (component.get("v.soldToAddrFromParent")) {
                        let addrId = component.get("v.soldToAddrFromParent.deliveryAddressId");
                        component.set("v.valueMap.soldAddress", addrId);
                    }

                }


                helper.initPaymentValues(component, event, helper);
    ...
    initPaymentValues: function (component, event, helper) {
    ...
    retriveContactsForZuora: function (component, event, helper, paymentMethodId) {
        ...
        var accId = component.get("v.firstRecordId");
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === "SUCCESS" && !$A.util.isEmpty(res)) {
                ...
                let sourceBP = component.get('v.billingProfileId');
                var valueMap = JSON.parse(res);

                if (sourceBP) {
                    helper.checkExistingEmptyBPLI(component, event, helper, valueMap, paymentMethodId, mapObj, 'createZuoraCustomer', '');
                } else {
    ...
    createZuoraAccount: function (component, event, helper, valueMap, paymentMethodId, mapObj, siaPayment, interactionId) {
        var accountId = component.get("v.firstRecordId");
        ...
        if (component.find("invoiceDeliveryPreferencesPick") != undefined) {
            var invoiceDel = component.find("invoiceDeliveryPreferencesPick").get("v.value"); //prefered delivery
        }
        ...
        //se sto aggiungendo un bpli su nuova LE

        let billingProfileId = component.get("v.billingProfileId");
        if (billingProfileId) {
            paymentMethodType = component.get("v.paymentAttr");
            component.set("v.valueMap.payment", paymentMethodType);
        }
        ...
        if (component.find("cupsId") != undefined) {
            var cupsId = component.find("cupsId").get("v.value");
        } else if (component.get("v.choosenContractCup")) {
        ...
        var ordId = component.get("v.configurationId"); //[Deav 21/12/2021 TPB: populate TPBCode for BillingProfileLineItem]
        ...
        var info = {
            ...
            isB2BG : isB2BG,
            ordId : ordId //[Deav 21/12/2021 TPB: populate TPBCode for BillingProfileLineItem]
        };
            ...
            if (state === "SUCCESS") {
                ...
                if (res.success && res.objectInfo != null && res.objectInfo != '') {
                    ...
                    if(res.typeMessage == 'BPLI error') {
                        console.log('BPLI error = ' + res.resultMessage);
                        component.set("v.showSpinner", false);
                        component.find('notifLib').showNotice({
                            "variant": "error",
                            "header": res.resultMessage,
                            "message": "",
                ...
                let errorMsg = response.getError()[0].message;
                console.log('ERROR ON CREATE BILLING PROFILE ' + errorMsg);
            }
    ...
    createUnlinkedZuora : function (component, event, helper, valueMap, paymentMethodId, mapObj, bpliId) { 
        ...
        var selectedValues;
        if (component.find("invoiceDeliveryPreferencesPick")) {
            selectedValues = component.find("invoiceDeliveryPreferencesPick").get("v.value");
        }
       ...
       var paymentTerm = 'Due Upon Receipt';
        ...
        } else if (paymentMethodType == 'Commodity Bill' && component.get("v.contractBatch") && component.get("v.getContractBy") === 'IdentityDoc') { // && !component.get("v.isB2BLE")
            if (selectedValues) {
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === "SUCCESS") {
                ...
                if (res.success == "true") {
                    ...
                    //component.get("v.sobjecttype") == 'Account' && mapObj.payment != 'CreditCardSIA'
                    if (fromBillingProfile || mapObj.payment != 'Credit Card') {
                ...
                let errorMsg = response.getError()[0].message;
                console.log('ERROR ON CREATE UNLINKED ZUORA ' + errorMsg);
    ...
    createPaymentMethod: function (component, event, helper, valueMap, zuoraAccountId, entityName, bpliId) {
        ...
        if (paymentMethodCategory === "External") {
            ...
            if (paymentMethodType === 'Direct Debt') {
                ...
                if (component.get("v.bankFromParent")) {
                    accountNumber = component.get("v.bankFromParent");
                } else if(component.find("accountNumber") != undefined) {
    ...
    checkValue: function (component, event, helper) {
        ...
        if (component.find("comboboxpayment") != undefined && !component.get("v.paymentAttr")) {
    ...
    checkValueForAccount: function (component, event, helper) {
        ...
        console.log('IN checkValueForAccount');
        ...
        }else if(paymentMethodType === 'External Financing' && !component.get("v.billingProfileId")){
        ...
        if ((component.get("v.valueMap.payment") && component.get("v.valueMap.legal") != undefined &&
            ...
            component.get("v.valueMap.soldAddress")) || true) {
            ...
            if (paymentMethodType === 'Direct Debt') {
                ...
                if (component.get("v.sobjecttype") === 'Account' && !component.get("v.billingProfileId")) {
                ...
                /*if(component.get("v.getContractBy") === 'POS'){ //if (component.get("v.isB2BLE")) {
                    //var contractId = component.find("contractId") ? component.find("contractId").get("v.value") : '';
                    //var cupsId = component.find("cupsId") ? component.find("cupsId").get("v.value") : '';
                    var cupsId = component.get("v.selectedPOS");
                    var contractId = component.get("v.valueMap.contractId"); //component.get('v.contractValue');
                } else {*/
                    var contractId = component.get("v.valueMap.contractId");
                    var cupsId = component.get("v.choosenContractCup");
                //}
    ...
    checkValueForBilling: function (component, event, helper) {
        ...
        console.log('IN checkValueForBilling');
        ...
        if (component.find("invoiceDeliveryPreferencesPick") && (component.get("v.invoiceDelValue") == undefined || component.get("v.invoiceDelValue") == '')) {
            component.set("v.disabledSubmit", true);
            return;
        }
    ...
    inizializeCreditCardFields: function (componet) {
        ...
        componet.setp("v.valueMap.month", "");
    ...
    retrieveCups: function (component, event, helper) {
        ...
        let soldAddressId = component.get('v.valueMap.soldAddress');
        if (soldAddressId != 'undefined' && soldAddressId != '') {
            component.set("v.showCommodity", true);
            var action = component.get("c.retrieveCups");
            action.setParams({
                "addressId": soldAddressId,
                "documentNumber": "" //non più necessario FI 2020-02-27
            });
            action.setCallback(this, function (response) {
                var state = response.getState();
                ...
                if((JSON.stringify(component.get("v.valueMap.billingAddresses")).includes('Brazil')) && component.get("v.valueMap.stream")=='B2C'){
                ...
                if (l == 0 && component.get("v.valueMap.payment") == 'Commodity Bill') {
                    if(!(JSON.stringify(component.get("v.valueMap.billingAddresses")).includes('Brazil')) || (JSON.stringify(component.get("v.valueMap.billingAddresses")).includes('Brazil') && component.get("v.valueMap.stream") !='B2C')){   // gaurav.tejpal@accenture.com, 14/07/2022,Enel X - Get Contract] -->
                    ...
                    else if(retValue.contract == "" && component.find("docNumber") != undefined
                    && (JSON.stringify(component.get("v.valueMap.billingAddresses")).includes('Brazil'))
                    ...
                    /*optsContr.push({
                        ...
                        value: retValue.contract,
                        label: retValue.contract
                    });
                    component.set('v.contractOptions', optsContr);*/

                    component.set('v.showCUPS', true);
                    component.set('v.showCommodity', true);
                    component.set('v.cupsList', opts);
                    component.set('v.isB2B', retValue.isB2B);
                    //console.log('CUPS LIST = ' + opts);
                    component.set('v.contractValue', retValue.contract);
                    component.set("v.valueMap.contractId", retValue.contract);
                    if(component.find("contractId")){
                        component.find("contractId").set("v.value", retValue.contract);
                    }
                    ...
                    component.set('v.valueMap.cupsId', opts[0].label);
                    //component.set("v.showSpinner", false);
                } else { // SERVER dei CUPS DOWN
                    console.log('ERROR ON GETTING CUPS:: ' + response.getError()[0].message);
                    //component.set("v.showSpinner", false);
                }
                component.set("v.showSpinner", false);
            });
    ...
    retrieveCupsByDocAndPOS: function (component, event, helper) {
        ...
        let selectedOptionValue = component.get('v.valueMap.soldAddress');
    ...
    checkExistingEmptyBPLI: function (component, event, helper, valueMap, paymentMethodId, mapObj, siaPayment, interactionId) {

        let action = component.get('c.manageUpdateUnusedBPLISoldTo');
        let taxExemptPickVal = component.find("taxExemptId") ? component.find("taxExemptId").get("v.value") : "";

        let paramsMap = {
            bpId: component.get('v.billingProfileId'),
            legalEntity: component.get("v.valueMap.legal"),
            soldToAddressId: valueMap.soldAddress,
            taxPick: taxExemptPickVal,
            billToAddressId: valueMap.billAddress
        }
        action.setParams({
            paramsMap: paramsMap
        });
            ...
            if (response.getState() === "SUCCESS") {
                ...
                if (result.success) {
                    ...
                    if (result.bpliFound) {
                        ...
                        console.log('NO UNUSED BPLI FOUND');
                        helper.createZuoraAccount(component, event, helper, valueMap, paymentMethodId, mapObj, siaPayment, interactionId);
                    }
                } else {
                    helper.showMessage(component, event, helper, result.errorMessage, 'error');
                    if (result.errorMessage) {
                        console.log('ERROR ON CHECK BPLI ON CALLBACK ' + result.errorMessage);
                    }
                }

            } else {
                let msg = response.getError()[0].message;
                console.log('ERROR ON checkExistingEmptyBPLI ' + msg);
            }
    ...
    getB2CCommodityContract: function (component, event, helper) {
       ...
       component.set("v.showSpinner", true);
       if((JSON.stringify(component.get("v.valueMap.billingAddresses")).includes('Brazil'))){
           for(let i = 0; i< component.get("v.valueMap.listPersonalDocument.length");i++ ){
               console.log(component.get("v.valueMap.listPersonalDocument[i].key"));
               var cfID = component.get("v.valueMap.listPersonalDocument["+i+"].key");
               if(cfID == component.find("selectDocument").get("v.value")){
                  docNum = component.get("v.valueMap.listPersonalDocument["+i+"].value");
                  console.log(docNum);
                  docNif = (component.get("v.valueMap.listPersonalDocument["+i+"].value")).split('- ')[0].replace(/\s+/g, ''); 
                  console.log(docNif);
                  docCountry = 'Brazil';
               }
           }
       }else{
           docNif = component.find('docNif').get('v.value');
           docNum = component.find('docNumber').get('v.value');        
           docCountry = component.find('docCountry').get('v.value'); 		
       }
       let accountId = component.get('v.firstRecordId');
       let billingProfileId = component.get("v.billingProfileId");
       ...
       if (!billingProfileId) {
           ...
           action.setCallback(this, function (response) {
               if (response.getState() === "SUCCESS") {
                   ...
                   if (result.length > 0) {
                       ...
                       if ((JSON.stringify(component.get("v.valueMap.billingAddresses")).includes('Brazil')) && component.get("v.recordTypeName") == 'XC_GLO_Account_Residential'){
                           component.set("v.getContractBy","IdentityDoc");
                       }
                   //[ START, gaurav.tejpal@accenture.com, 13/04/2022,Enel X - Get Contract] -->
                   }else if(component.get("v.recordTypeName") == 'XC_GLO_Account_Residential' && (JSON.stringify(component.get("v.valueMap.billingAddresses")).includes('Brazil')))
                       {
                       var toastEvent = $A.get("e.force:showToast");
                       var title = $A.get("$Label.c.XC_CL_ErrorsOccurred");
                       var message = $A.get("$Label.c.XC_CL_NoContractForDoc");
           ...
           let action = component.get("c.getExistingCommodityContractInfos");
           action.setParams({
               "billingProfileId": billingProfileId
           });
           action.setCallback(this, function (response) {
               if (response.getState() === "SUCCESS") {
                   let result = response.getReturnValue();
                   component.set("v.valueMap.contractId", result.contractId);
                   component.set("v.choosenContractCup", result.contractCups);
                   component.set("v.contractBatch", result.billingBatch);
                   component.set("v.choosenContractStatus", "Active");
                   component.set("v.showSpinner", false);
               } else {
                   let msg = response.getError()[0].message;
                   console.log("ERROR RETRIEVING EXISTING COMMODITY CONTRACT " + msg);
               }
           });
