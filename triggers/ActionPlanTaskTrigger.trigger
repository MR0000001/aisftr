trigger ActionPlanTaskTrigger on Task ( after update, after delete , before delete, before insert) {
	
	//NA trigger on before Insert
    if (trigger.isInsert && trigger.isBefore){
		ActionPlansTaskTriggerUtilities.beforeInsert(trigger.new);
	}
